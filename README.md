...
Одна из особенностей функционального программирования -- работа с чистыми функциями.
Это функции в математическом смысле: их возвращаемое значение зависит только от
значений входных параметров, но НЕ зависит от места и времени их вызова.

Побочные эффекты в Haskell реализованы с помощью полиморфного контейнера `IO`. Реализация
этого типа данных основывается на сложных математических абстракциях, так что просто
будем считать, что где-то есть объявление типа

```haskell
data IO a = ...
```

Для того, чтобы "завернуть" значение в контейнер `IO`, используйте функцию `return :: a -> IO a`.
Пути в обратную сторону не существует: если какая-то часть программы работает с побочными
эффектами, то и вся программа работает с побочными эффектами. Если вы нашли функцию
`unsafePerformIO :: IO a -> a`, то её использовать не рекомендуется (она даже в названии
предупреждает о том, что это небезопасно).

Любое приложение можно разделить на чистые функции и работу с побочными эффектами.
При реализации практического задания постарайтесь по возможности использовать чистые функции.
